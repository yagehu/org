...
=apply L in H= matches some conditional statement =L= (of the form =X
-> Y=, say) against a hypothesis =H= in the context.  However, unlike
ordinary =apply= (which rewrites a goal matching =Y= into a subgoal
=X=), =apply L in H= matches =H= against =X= and, if successful,
replaces it with =Y=.

In other words, =apply L in H= gives us a form of "forward reasoning":
